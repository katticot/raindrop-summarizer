...
import { Config, CLIOptions } from "../types.ts";
...
export class ConfigManager {
	...
	private validateConfig(config: Config): void {
		...
		if (!config.raindropToken) {
			errors.push("RAINDROP_TOKEN is required. Get it from https://app.raindrop.io/settings/integrations");
		}

		if (!config.googleCloudProjectId) {
			errors.push("GOOGLE_CLOUD_PROJECT_ID is required. Set up a Google Cloud project with Vertex AI enabled");
		}
