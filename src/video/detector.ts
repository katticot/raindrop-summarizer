...
export class VideoDetector {
	...
	isVideoUrl(url: string): boolean {
		try {
			...
			return this.supportedSites.some((site) =>
				site.hostnames.some((supportedHostname) => hostname.includes(supportedHostname))
			);
		} catch (error) {
	...
	filterVideoBookmarks(bookmarks: RaindropItem[]): RaindropItem[] {
		...
		this.logger.debug(
			`Filtered ${bookmarks.length} bookmarks, found ${videoBookmarks.length} video URLs`,
		);
	...
	generateFilename(url: string, title?: string): string {
		...
		try {
			...
			// Final fallback
			const safeName = title ? title.replace(/[^a-zA-Z0-9_-]/g, "_").substring(0, 50) : "video";
